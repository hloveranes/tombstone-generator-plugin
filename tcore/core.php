...
class TombCore {
    ...
    public function ts_custom_enqueue(){
        ...
        wp_enqueue_style('ts-gen-tyle', plugin_dir_url(dirname( __FILE__ )).'app/css/baseStyle.css', array(), '1.4.2', 'all' );
        //wp_enqueue_style('string $handle', mixed $src, array $deps, mixed $ver, bol $in_footer );
        wp_enqueue_script('ts-gen-contextJSON', plugin_dir_url(dirname( __FILE__ )).'app/js/contextJSON.js', array(), '3.1.1', 'true' );
        wp_enqueue_script('ts-gen-dragndrop', plugin_dir_url(dirname( __FILE__ )).'app/js/dragndrop.js', array(), '3.1.2', 'true' );
        wp_enqueue_script('ts-gen-editor', plugin_dir_url(dirname( __FILE__ )).'app/js/editor.js', array(), '3.2.3', 'true' );
